@extends('Home.public.base') @section('title',"商品详情") @section('main')
<div class="container">
    <div class="row">
        <div class="col-md-5">
            @foreach($images as $v)
            <img src="{{asset('uploads')}}/{{$v->path}}" class="col-md-12" alt="{{$product->title}}" /> @endforeach
        </div>
        <div class="col-md-7">
            ...
            <div>
                ...
                <div>购买数量：
                    <input name="number" type="text" id="number" value="1" size="4" autocomplete="off" />
                </div>
            </div>
            <div style="margin: 60px;"></div>
            <div>
                {{csrf_field()}} @if($collectListid)
                <a href="javascript:delPro({{$product->id}})" class="btn btn-danger btn-disfavor">取消收藏</a> @else
                <a href="javascript:addCollect({{$product->id}})" class="btn btn-success btn-favor">❤ 收藏</a> @endif
